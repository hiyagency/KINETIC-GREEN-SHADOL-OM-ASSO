...
const SHOWROOM_CONTACT = "9425016493";
...
export function StickyMobileCTA({ settings }: { settings: StoreSettings }) {
  ...
  const actions = [
    ...
    {
      label: "Contact",
      href: `tel:${SHOWROOM_CONTACT}`,
      icon: <Phone size={20} />,
      className: "bg-[#101510] text-white",
    },
    {
      label: "Instagram",
      href: instagramUrl,
  ...
  return (
    ...
    >
      <div className="mx-auto grid max-w-xl grid-cols-6 gap-1.5 md:flex md:max-w-none md:gap-2">
        {actions.map((action) => (
          <a
            key={action.label}
            href={action.href}
            aria-label={action.label === "Contact" ? "Contact Showroom" : action.label}
            className={`flex min-h-14 min-w-0 flex-col items-center justify-center gap-1 rounded-2xl px-1.5 text-[10px] font-black transition hover:-translate-y-0.5 hover:shadow-lg active:translate-y-0 md:min-h-11 md:flex-row md:rounded-full md:px-4 md:text-sm ${action.className}`}
          >
            {action.icon}
